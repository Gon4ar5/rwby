class SenderService
	...
	def self.send_third_get_request(session)
	  uri = URI.parse("https://pass.rw.by/en/")
	  request = Net::HTTP::Get.new(uri)
	...
	def self.get_route_params(session)
	  choose_route_link = "https://pass.rw.by/en/route/?from=Minsk+Pasa%C5%BEyrski&from_exp=#{SenderService::STATION[:minsk]}&from_esr=&to=Pinsk&to_exp=#{SenderService::STATION[:pinsk]}&to_esr=133202&date=#{set_date}"
	...
	def self.send_ajax_req_for_free_places_hash(session)
	  choose_type_of_carriage_link = "https://pass.rw.by/en/ajax/route/car_places/?from=#{SenderService::STATION[:minsk]}&to=#{SenderService::STATION[:pinsk]}&date=#{set_date}&train_number=657%D0%91&car_type=4&from_time=#{set_date_time}&_=#{Time.now.to_i.to_s + '000'}"
	...
	def self.send_ajax_req_for_train_pixels(free_places_hash, session)
	...
	def self.send_request_with_seat_info(free_places_hash, second_ajax_resp, session)
	...
	def self.send_passanger_info(session)
	  uri = URI.parse("https://pass.rw.by/en/order/passengers/")
	  request = Net::HTTP::Post.new(uri)
	...
	def self.send_orders_request(session)
